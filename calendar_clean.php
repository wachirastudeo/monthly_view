...
<dl class="mec-calendar-row">
    ...
    for ($list_day = 1; $list_day <= $days_in_month; $list_day++) {
        ...
        // Print events 
        if (isset($events[$today]) and count($events[$today])) {
            echo '<dt class="mec-calendar-day mec-has-event' . $selected_day . '" data-mec-cell="' . $day_id . '" data-day="' . $list_day . '" data-month="' . date('Ym', $time) . '">';
            // date
            echo ' <a href="' . ($this->display_all ? '#mec-calendar-events-sec-' . $this->id . '-' . $day_id : '#') . '" class="mec-has-event-a">' . $list_day . '</a>';

            //insert before display_all
            foreach ($events[$today] as $event) {

                $event_id = $event->data->ID;
                $event_title = $event->data->title;
                $event_excerpt = wp_trim_words(get_the_excerpt($event_id), 20, '...');
                $event_thumbnail = get_the_post_thumbnail($event_id, 'large');
                $event_date = date_i18n('l, d M Y', strtotime($event->data->date['start']['date']));

                echo '<div class="mec-month-event" 
            ...
            // Popup HTML (อยู่นอก Loop)
            echo '<div id="mec-event-overlay" class="mec-popup-overlay"></div>';
    ...
    if ($days_in_this_week < 8) {
        for ($x = 1; $x <= (8 - $days_in_this_week); $x++) {
            ...
            // Print events
            if (isset($events[$today]) and count($events[$today])) {
                echo '<dt class="mec-calendar-day mec-has-event' . $selected_day . '" data-mec-cell="' . $day_id . '" data-day="' . $list_day . '" data-month="' . date('Ym', strtotime($year . '-' . $month . '-01')) . '"><a href="' . ($this->display_all ? '#mec-calendar-events-sec-' . $this->id . '-' . $day_id : '#') . '" class="mec-has-event-a">' . $list_day  .
